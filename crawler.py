...
무엇을 보려는 도구인가
 어떤 캐릭터 IP가, 어떤 기종에서, 어떤 카테고리(케이스/맥세이프지갑/그립톡...)로
 잘 팔리고 있는지를 본다. IP 소싱·기획 판단용이라 월 1회 정도면 충분하다.

동작 순서
 1. Supabase에서 수집 키워드(crawl_keywords)와 캐릭터 사전(character_dict)을 불러옴
    - 키워드마다 기종(device)과 카테고리(category)가 선언되어 있다
...
기존 노트북 대비 수정 사항
 ...
 - pandas/numpy/openpyxl 의존성 제거 (Windows 앱 제어 정책의 DLL 차단 회피)
 - 네이버 차단 페이지 감지 시 즉시 중단, 페이지 간격 랜덤화
 - 수집 회차 기록은 실제로 상품을 모은 뒤에 생성 (빈 회차가 남지 않음)
 - 기종/카테고리를 축으로 저장, 상품 링크와 상품ID를 함께 저장
   (회차 간 같은 상품을 매칭해 '리뷰 증가분'을 계산하기 위함)
"""
...
# 설정
# ------------------------------------------------------------
ITEMS_PER_KEYWORD = getattr(config, "ITEMS_PER_KEYWORD", 150)  # 키워드당 목표 수집 개수
ITEMS_PER_PAGE = 20
...
BETWEEN_PAGES = getattr(config, "BETWEEN_PAGES", (4, 9))       # 페이지 간 추가 대기
BETWEEN_KEYWORDS = getattr(config, "BETWEEN_KEYWORDS", (15, 30))  # 키워드 간 추가 대기
...
FIELDS = [
    "crawled_at", "keyword", "device", "category", "product_name", "character_name",
    "rating", "reviews", "purchases", "wishes", "reg_date", "months_on_sale",
    "point", "product_id", "product_url",
]
...
def load_keywords():
    """키워드마다 기종/카테고리가 함께 온다. 없으면 None."""
    rows = sb_select("crawl_keywords", {
        "select": "keyword,device,category",
        "is_active": "eq.true",
        "order": "sort_order.asc",
    })
    if not rows:
        fallback = getattr(config, "FALLBACK_KEYWORDS", ["캐릭터 케이스"])
        print(f"[안내] DB에 활성 키워드가 없어 기본 키워드를 사용합니다: {fallback}")
        return [{"keyword": k, "device": None, "category": None} for k in fallback]
    return rows
...
def extract_product_id(url):
    """네이버 상품 링크에서 상품 ID를 뽑는다. 회차 간 같은 상품을 잇는 열쇠."""
    if not url:
        return None
    for pat in (r"/catalog/(\d+)", r"/products/(\d+)", r"[?&]nvMid=(\d+)", r"/(\d{8,})(?:[/?#]|$)"):
        m = re.search(pat, url)
        if m:
            return m.group(1)
    return None
...
def read_card(card):
    """상품 카드 하나에서 제목 / 판매정보 / 링크를 뽑는다."""
    title = card.find_element(By.CSS_SELECTOR, "div.product_title__Mmw2K").text
    amt = card.find_element(By.CSS_SELECTOR, "div.product_etc_box__ElfVA").text

    link = ""
    for sel in ("a[class*='product_link']", "a[href]"):
        els = card.find_elements(By.CSS_SELECTOR, sel)
        if els:
            link = els[0].get_attribute("href") or ""
            break
    return title, amt, link
    ...
    for page in range(1, total_pages + 1):
        ...
        for i in range(1, ITEMS_PER_PAGE + 1):
            ...
            try:
                ...
                title, amt, link = read_card(card)
            except Exception:
                misses += 1
                continue

            # 상품ID가 있으면 그걸로, 없으면 상품명으로 중복 판정
            key = extract_product_id(link) or title
            if not title or key in seen:
                continue
            seen.add(key)
            collected.append((title, amt, link))
            page_found += 1
...
def build_rows(kw, raw_items, char_dict_sorted, run_id):
    today = date.today().isoformat()
    rows = []
    for title, amt, link in raw_items:
...
def print_blocked_notice(detail):
    ...
    print("  - 다시 할 때는 config.py 의 ITEMS_PER_KEYWORD 를 100~150 정도로")
...
def describe(kw):
    bits = [kw["keyword"]]
    tags = [t for t in (kw.get("device"), kw.get("category")) if t]
    if tags:
...
def main():
    ...
    driver = make_driver()
    harvest = []          # (키워드dict, 원본항목들)
    blocked = None
    try:
        for idx, kw in enumerate(keywords):
            print(f"\n>> {describe(kw)} 수집 중...")
            try:
                raw = crawl_keyword(driver, kw["keyword"], ITEMS_PER_KEYWORD)
            except NaverBlocked as e:
                blocked = str(e)
                break
            harvest.append((kw, raw))
            if idx < len(keywords) - 1:
                nap(BETWEEN_KEYWORDS)
    finally:
        driver.quit()
    ...
    run = sb_insert("crawl_runs",
                    {"keywords": [kw["keyword"] for kw, _ in harvest], "item_count": 0},
                    returning=True)[0]
    run_id = run["id"]

    all_rows = []
    for kw, raw in harvest:
        all_rows.extend(build_rows(kw, raw, char_dict_sorted, run_id))
